...
namespace ITProjectPriceCalculationManager.ITProjectsManager.API.Core.Services
{
    internal class ApplicationInfoForEvaluationService : IApplicationInfoForEvaluationService
    {
        ...
        {
            var applicationForEvaluation = _mapper.Map<EvaluationDTO>((await _applicationForEvaluationRepository.ExecuteStoredProcedure($"EXEC dbo.GetApplicationsByIdForEvaluation {applicationId}")).FirstOrDefault());
            ...
            applicationForEvaluation.EvaluationFactors = (from af in _dbContext.ApplicationToFactors
                                                          join dltft in _dbContext.DifficultyLevelsTypeToFactorTypes on af.DifficultyLevelsTypeToFactorTypeId equals dltft.Id
                                                          ...
                                                          select new EvaluationFactorDTO()
                                                          {
                                                              ...
                                                              FactorType = (from ft in _dbContext.FactorTypes
                                                                            ...
                                                                            select ft).First().Id == new Guid("B03771E5-488A-449F-B886-19C581B63CDE") ? DTOModels.Enums.FactorType.InfluenceFactors : DTOModels.Enums.FactorType.ScaleFactors
                                                          }).ToList();

            applicationForEvaluation.ProgramsParametrEvaluationFactor = _mapper.Map<List<ProgramsParametrEvaluationFactorDTO>>(await _programsParametrRepository.GetListBySpecAsync(new ProgramsParametrs.ProgramsParametrsByApplicationId(applicationId)));
            ...
            applicationForEvaluation.ProgramsParametrEvaluationFactor
                .ForEach(ppef => ppef.SubjectAreaElements = (from pp in _dbContext.ProgramsParametrs
                                                             join pptsae in _dbContext.ProgramsParametrToSubjectAreaElements on pp.Id equals pptsae.ProgramsParametrId
                                                             ...
                                                             select new EvaluationFactorDTO()
                                                             {
                                                                 ...
                                                                 FactorType = (from ft in _dbContext.FactorTypes
                                                                               where ft.Id == dltft.FactorTypeId
                                                                               select ft).First().Id == new Guid("CDD64D87-BB9A-4C17-B809-05C4454E6998") ? DTOModels.Enums.FactorType.InformationObject : DTOModels.Enums.FactorType.Function
                                                             }).ToList());
