...
internal class AlbrehtMethod : CalculateMethod
{
    public override async Task<EvaluationResultDTO> Calculate(EvaluationDTO evaluation, double? price)
    {
        ...
        try
        {
            foreach (var programsParametr in evaluation.ProgramsParametrEvaluationFactor)
                ksloc += programsParametr.SLOC * CountUOF(GetFactors(programsParametr.SubjectAreaElements),
                    evaluation.ConfidenceArea, tehnique);
    ...
    private double CountUOF(IEnumerable<IGrouping<Guid, EvaluationFactorDTO>> subjectAreaElements,
        double? confidenceArea, BaseDelphiTechnique tehnique)
    {
        double sum = 0;
    ...
    {
        return 152 * evaluation.AverageCostLabor *
            CountDevelopmentAverageComplexity(evaluation.EvaluationFactors, ksloc, evaluation.ConfidenceArea,
                tehnique) / evaluation.AverageMonthlyRateWorkingHours;
    }

    private double CountDevelopmentAverageComplexity(List<EvaluationFactorDTO> evaluations, double ksloc,
        double? confidenceArea, BaseDelphiTechnique tehnique)
    {
        double productInfluenceFactors = 1;

        foreach (var influenceFactors in
                 GetFactors(evaluations.Where(e => e.FactorType == FactorType.InfluenceFactors)))
        {
            var result = tehnique.CheckFactors(influenceFactors.ToList(), confidenceArea);

            if (!string.IsNullOrEmpty(result.Error)) return 0;

            productInfluenceFactors = productInfluenceFactors * result.FactorValue;
        }

        return 2.94 * productInfluenceFactors * Math.Pow(ksloc,
            CountB(GetFactors(evaluations.Where(e => e.FactorType == FactorType.ScaleFactors)), confidenceArea,
                tehnique));
    }

    private double CountB(IEnumerable<IGrouping<Guid, EvaluationFactorDTO>> evaluations, double? confidenceArea,
        BaseDelphiTechnique tehnique)
    {
