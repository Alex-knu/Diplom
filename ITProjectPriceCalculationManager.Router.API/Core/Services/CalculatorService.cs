...
internal class CalculatorService : ICalculatorService
{
    ...
    {
        var application =
            await _routeService.GetByIdAsync<BaseApplicationDTO, Guid>(_itManagerClient, "baseapplicationapi",
                applicationId);
        var applicationInfo = await _routeService.GetByIdAsync<EvaluationDTO, Guid>(_itManagerClient,
            "applicationinfoforevaluationapi", applicationId);
        var result =
            await _routeService.PostAsJsonAsync<EvaluationDTO, EvaluationResultDTO>(_calculatorClient, "calculateapi",
                applicationInfo);
        ...
        return await _routeService.PutAsJsonAsync<BaseApplicationDTO, BaseApplicationDTO>(_itManagerClient,
            "baseapplicationapi", application);
    }
}